...
# a main offline dinastycoind, does most of the tests
# a restricted RPC dinastycoind setup with RPC payment
# two local online dinastycoinds connected to each other
N_DINASTYCOINDS = 4

# 4 wallets connected to the main offline dinastycoind
# a wallet connected to the first local online dinastycoind
N_WALLETS = 5

WALLET_DIRECTORY = builddir + "/functional-tests-directory"
DIFFICULTY = 10

dinastycoind_base = [builddir + "/bin/dinastycoind", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "dinastycoind_p2p_port", "--rpc-bind-port", "dinastycoind_rpc_port", "--zmq-rpc-bind-port", "dinastycoind_zmq_port", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "dinastycoind_data_dir", "--log-level", "1"]
dinastycoind_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/dinasty-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1"]
wallet_extra = [
  ["--daemon-port", "37175"],
  ["--daemon-port", "37175"],
  ["--daemon-port", "37175"],
...
for i in range(N_DINASTYCOINDS):
  command_lines.append([str(37175+i) if x == "dinastycoind_rpc_port" else str(18280+i) if x == "dinastycoind_p2p_port" else str(18380+i) if x == "dinastycoind_zmq_port" else builddir + "/functional-tests-directory/dinastycoind" + str(i) if x == "dinastycoind_data_dir" else x for x in dinastycoind_base])
