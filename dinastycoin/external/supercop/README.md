...
serve as a more complex example of its usage. The project directory structure:
  ...
  - **intree.cmake** - Creates a cmake library target `dinastycoin-crypto-intree`
    and generates a header at `<BUILD_LOCATION>/include/dinastycoin/crypto.h`.
  - **CMakeLists.txt** - Declares a new project for creating/installing a
    shared or static `dinastycoin-crypto` library.

> Downstream projects cannot currently use this project _without_ also using
> the `dinastycoin-project/dinastycoin/src/crypto/crypto.h` functions - specifically
...
Every cryptographic library implements:
  ...
  - **`dinastycoin_crypto_<namespace>_scalarmult(char* out, const char* pub, const char* sec)`** -
    ...
    ed25519 curve.
  - **`dinastycoin_crypto_<namespace>_generate_key_derivation(char* out, const char* tx_pub, const char* view_sec)`** -
    where `out` is the shared secret, `tx_pub` is the public key from the
    transaction and `view_sec` is the users view-key secret.
  - **`dinastycoin_crypto_<namespace>_generate_subaddress_public_key(char* out, const char* output_pub, const char* special_sec)`** -
...
auto-generated header and use `dinastycoin_crypto_generate_key_derivation`, etc.
...
dinastycoin_crypto_get_target("amd64-64-24k" CRYPTO_TARGET)
...
add_library(all $<OBJECT_TARGETS:dinastycoin-crypto-intree> all.cpp)
add_library(referenced referenced.cpp)
target_link_libraries(referenced dinastycoin-crypto-intree)
```
...
A header file is written to `<BUILD_LOCATION>/include/dinastycoin/crypto.h` which
