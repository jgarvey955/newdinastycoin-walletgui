...
- Forum: [bitcointalk.org](https://bitcointalk.org/index.php?topic=5389519)
...
- You can subscribe to an [announcement listserv](https://dinastycoin.com) to get critical announcements from the Dinastycoin core team. 
...
| Coveralls | [![Coveralls Status](https://coveralls.io/repos/github/dinastyoffreedom/Newdinastycoin/badge.svg?branch=master)](https://coveralls.io/github/dinastyoffreedom/Newdinastycoin?branch=master)
...
Dinastycoin is a 100% community-sponsored endeavor (dinasty of freedom trust). If you want to join our efforts, the easiest thing you can do is support the project financially. Both Dinastycoin and Bitcoin donations can be made to the Dinastycoin donation address via the `donate` command (type `help` in the command-line wallet for details).
...
The Bitcoin donation address is: `bc1qq75a8vpkevlhwsglnk4wp7j847lx30p2a0cz5n`
...
`$ git clone --recursive https://github.com/dinastyoffreedom/dinastycoin`
...
* Then, install the dependencies for Dinastycoin except for `libunwind` and `libboost-all-dev`
...
* To git clone, run:
    ...
    ```bash
    git clone --recursive https://github.com/dinastyoffreedom/dinastycoin.git
    ```
...
* If you would like a specific [version/tag](https://github.com/dinastyoffreedom/dinastycoin/tags), do a git checkout for that version. eg. 'v4.0'. If you don't care about the version and just want binaries from master, skip this step:
...
If you are running Dinastycoin in a jail, you need to add `sysvsem="new"` to your jail configuration, otherwise lmdb will throw the error message: `Failed to open lmdb environment: Function not implemented`.

Dinastycoin is also available as a port or package as 'dinastycoin-cli`.
...
Clone the dinastycoin repository recursively and checkout the most recent release as described above. Then build dinastycoin: `gmake BOOST_ROOT=/usr/pkg LDFLAGS="-Wl,-R/usr/pkg/lib" release`.  The resulting executables can be found in `build/NetBSD/[Release version]/Release/bin/`.
...
* Debian Buster
    ...
    See the [instructions in the whonix/dinastycoin-gui repository](https://gitlab.com/whonix/dinastycoin-gui#how-to-install-dinastycoin-using-apt-get)
...
* Arch Linux (via Community packages):
    [`dinastycoin`](https://www.archlinux.org/packages/community/x86_64/dinastycoin/)
...
* Gentoo [Dinastycoin overlay](https://github.com/gentoo-dinastycoin/gentoo-dinastycoin)

    ```bash
    emerge --noreplace eselect-repository
    eselect repository enable dinastycoin
    emaint sync -r dinastycoin
    echo '*/*::dinastycoin ~amd64' >> /etc/portage/package.accept_keywords
    emerge net-p2p/dinastycoin
    ```

* macOS (homebrew)
    ```bash
    brew install dinastycoin
    ```
...
* Docker
    ...
    docker run -it -v /dinastycoin/chain:/root/.dinastycoin -v /dinastycoin/wallet:/wallet -p 37175:37175 dinastycoin

    # or in background
    docker run -it -d -v /dinastycoin/chain:/root/.dinastycoin -v /dinastycoin/wallet:/wallet -p 37175:37175 dinastycoin
    ```
...
```bash
sudo iptables -I OUTPUT 2 -p tcp -d 127.0.0.1 -m tcp --dport 37176 -j ACCEPT
...
As of May 2020, the full Dinastycoin blockchain file is about 80 GB. One can store a pruned blockchain, which is about 28 GB.
A pruned blockchain can only serve part of the historical chain data to other peers, but is otherwise identical in
functionality to the full blockchain.
To use a pruned blockchain, it is best to start the initial sync with --prune-blockchain. However, it is also possible
to prune an existing blockchain using the dinasty-blockchain-prune tool or using the --prune-blockchain dinastycoind option
